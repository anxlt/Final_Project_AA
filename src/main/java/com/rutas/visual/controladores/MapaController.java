            ...
            for (Ruta ruta : ServicioGrafo.get().obtenerVecinos(desde)) {
                if (ruta.getDestino().equals(hacia)) {
                    ...
                    if (pt  instanceof Number)
                        totalTiempo      += ((Number) pt).doubleValue();
                    if (pc  instanceof Number)
                        totalCosto       += ((Number) pc).doubleValue();
                    if (pd  instanceof Number)
                        totalDistancia   += ((Number) pd).doubleValue();
                    if (ptr instanceof Number)
                        totalTransbordos += ((Number) ptr).doubleValue();
                    break;
                }
            }
        }
